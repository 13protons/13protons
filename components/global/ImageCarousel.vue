<template>
<div class="relative">
  <div v-if="selected" class="fixed z-50 top-0 left-0 bg-slate-900/60 w-screen h-screen" @click="close">
    <button ref="overlayControl" class="absolute ring-0 ring-transparent outline-none right-0 text-white/80 hover:text-cyan-500 rounded-full mr-4 md:mr-12 mt-4 md:mt-12 cursor-pointer rotate-45 align-middle bg-white/80 dark:bg-black/50 block pt-2 px-1.5 pb-1" @click="close" @keyup.left="prev()" @keyup.right="next()" @keyup.space="close">
      <svg
        class="w-8 h-8 dark:fill-amber-500 fill-sky-600 drop-shadow-md hover:drop-shadow-xl"
