...
<div class="relative">
  ...
  <div class="absolute right-0 text-white/80 hover:text-cyan-500 rounded-full mr-4 md:mr-12 cursor-pointer rotate-45 align-middle" @click="close">
      <svg
    ...
    <div v-if="selected.alt" class="absolute bottom-0 w-full bg-gray-200 text-slate-900 p-4 text-center pb-8 -ml-4 md:-ml-12">
      {{selected.alt}}
    </div>
  </div>
  <div class="flex flex-row flex-wrap">
    <div 
        v-for="image in imagePaths" :key="image"
        class="w-24 h-24 bg-slate-500 mb-2 mr-2 rounded-lg border-2 border-slate-300 dark:border-slate-900 hover:border-cyan-500 dark:hover:border-amber-500 transition-border-color duration-100 transition-translate hover:-translate-y-1 active:translate-y-0 cursor-pointer bg-cover bg-center bg-no-repeat"
